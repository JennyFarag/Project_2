...
import '../styles/FacetedSearch.css'; // Import the CSS file
...
const FacetedSearch = ({ onSearch }) => {
  ...
  const handleSearch = () => {
    onSearch({ language: selectedLanguage, material: selectedMaterial, level: selectedLevel });
  };
    ...
    <div>
      <div className="faceted-search-container">
        <Row className="faceted-search-row align-items-center">
          ...
          <Col className="faceted-search-col d-flex align-items-center">
            <Form.Label className="faceted-search-label">Material</Form.Label>
            <DropdownButton title={selectedMaterial || 'Select Material'} variant="outline-secondary" className="faceted-search-dropdown">
              {['Videos', 'Documents'].map(material => (
                <Dropdown.Item
                  as="button"
                  key={material}
                  onClick={() => handleCheckboxChange(setSelectedMaterial, material.toLowerCase())}
                  className="dropdown-item-checkbox"
                >
                  <Form.Check
                    type="checkbox"
                    ...
                    onChange={() => handleCheckboxChange(setSelectedMaterial, material.toLowerCase())}
                  />
                </Dropdown.Item>
              ))}
            </DropdownButton>
